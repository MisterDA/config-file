...
module Raw = struct
  ...
  module Parse = struct
    ...
    let rec file l (strm__ : _ Stream.t) =
      ...
      | Some id -> (
          ...
          | Some (Genlex.Kwd "=") -> (
              ...
              in
              try file ((id, v) :: l) strm__
              with Stream.Failure -> raise (Stream.Error ""))
          | _ -> raise (Stream.Error ""))
      | _ -> List.rev l

    and value (strm__ : _ Stream.t) =
      match Stream.peek strm__ with
      | Some (Genlex.Kwd "{") -> (
          Stream.junk strm__;
          let v =
            try file [] strm__ with Stream.Failure -> raise (Stream.Error "")
          in
          match Stream.peek strm__ with
          | Some (Genlex.Kwd "}") ->
              Stream.junk strm__;
      ...
      | Some (Genlex.Kwd "[") ->
          Stream.junk strm__;
          let v =
            try list [] strm__ with Stream.Failure -> raise (Stream.Error "")
          in
          List v
      | Some (Genlex.Kwd "(") ->
          Stream.junk strm__;
    ...
    and list l (strm__ : _ Stream.t) =
      match Stream.peek strm__ with
      | Some (Genlex.Kwd ";") -> (
          Stream.junk strm__;
          try list l strm__ with Stream.Failure -> raise (Stream.Error ""))
      | Some (Genlex.Kwd ",") -> (
          Stream.junk strm__;
          try list l strm__ with Stream.Failure -> raise (Stream.Error ""))
      | _ -> (
          match try Some (value strm__) with Stream.Failure -> None with
          | Some v -> (
              try list (v :: l) strm__
              with Stream.Failure -> raise (Stream.Error ""))
          | _ -> (
  ...
  let of_channel in_channel =
    let result =
      in_channel |> Stream.of_channel |> Parse.lexer |> Parse.file []
    in
    close_in in_channel;
    result
end
...
class group =
  object (self)
    ...
    method read_string ?obsoletes ?(no_default = false)
      ...
      in
      let raw_of_string s =
        s |> Stream.of_string |> Raw.Parse.lexer |> Raw.Parse.file []
      in
      self#read_from ?obsoletes no_default on_type_error
...
class ['a] cp_custom_type wrappers ?(group : group option) name ?short_name
  ...
  object (self)
    ...
    method get_formatted formatter =
      self#get |> self#to_raw |> Raw.save formatter

    method get_default_formatted formatter =
      self#get_default |> self#to_raw |> Raw.save formatter
